...
class LinkedList {
	...
	public String toString() {
		String msg = "";
		Node head = headNode.getNextNode();
		Student std_array[] = new Student[numOfData];	// 역순 출력을 위한 배열
		
		// 역순 출력을 위한 배열에 링크드 리스트 정방향으로 담기
		for (int i = numOfData-1; i >= 0; i--) {
			std_array[i] = head.getValue();
			head = head.getNextNode();
		}
		...
		for(int i = 0; i < numOfData; i++) {
			msg += "\t" + std_array[i].getNum();
			msg += "\t" + std_array[i].getId();
			msg += "\t" + std_array[i].getKor();
			msg += "\t" + std_array[i].getEng();
			msg += "\t" + std_array[i].getMath();
			msg += "\t" + std_array[i].getSum();
			msg += "\t" + std_array[i].getAvg();
			msg += "\t" + std_array[i].getRank();
